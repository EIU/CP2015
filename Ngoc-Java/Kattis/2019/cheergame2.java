...
public class cheergame2 {

	static int[][][] currentStatus = new int[19][19][81];
	static int[][][] nextStatus = new int[19][19][81];
				...
				for (int s = 0; s < 19; s++) {
					for (int p = 0; p < 81; p++) {
						...
						if (preValue <= total) {
							...
							if (p == 80 && (i < 45 || i >= 49) && n > nextCheer) {
								setStatus(f + 1, s, 27, preValue + nextCheer + 1);
								s2 = true;
							}
							if (p == 0 && (i < 45 || i >= 49) && nextCheer > 0) {
								setStatus(f, s + 1, 27, preValue);
								s0 = true;
							}
							if (!s2 && n > nextCheer) {
								setStatus(f, s, p2, preValue + nextCheer + 1);
							}
							if (n >= nextCheer) {
								setStatus(f, s, p1, preValue + nextCheer);
							}
							if (!s0 && nextCheer > 0) {
								setStatus(f, s, p0, preValue);
							}
						}
					}
				}
