registerProblem('EIUPOA15_EIMAX',
    ...
    function () {
        ...
        var maxValues = initValues();
        var maxValuesArray = [initValues(), initValues(), initValues(), initValues()];

        function setMaxValues(maxValues, d, i, next) {
            var vi = maxValues[i];
            var vNext = maxValues[next];
            if (vi.len < vNext.len + 1
                || (vi.len == vNext.len + 1 && vi.num < d)
                || vi.len == vNext.len + 1 && vi.num == d && vi.next > next
                ) {
                vi.len = vNext.len + 1;
                vi.num = d;
                vi.next = next;
